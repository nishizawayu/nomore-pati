...
import { updateNameSchema, UpdateNameInput } from "../validations";
...
export function EditNameForm({ currentName }: Props) {
    ...
    return (
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-900 font-medium">{currentName}</p>
        </div>
        <button
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
      <div>
        <label className="block text-sm text-gray-500 mb-1">ユーザー名</label>
        <input
          type="text"
          {...register("name")}
